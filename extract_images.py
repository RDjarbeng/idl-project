...
def get_non_zero_bb(img):
    ...
    a = np.where(img != 0)
    # cv2.imwrite(join('manipulated_sequences/FaceSwap/c40/mask',
    #                                  '_test_'), a)
...
def create_images_from_single_folder(data_path, output_path,
    ...
    print(original_data_path, '= original path')
    ...
    assert ([filename[:3] for filename in altered_filenames] ==
            [filename[:3] for filename in original_filenames]), \
    ...
    for i in range(len(altered_filenames)):
        ...
        if crop or return_masks:
            ...
            print('MASK DATA PATH', mask_data_path, 'MASK FILENAME', mask_filename)
